...
Lock 和 Latch 概念区别
1. Lock是更高层次的逻辑概念，Latch是低层次的概念（实现细节）
2. Lock主要保护元组、表和数据库，Latch主要保护内部数据结构
3. Lock在整个事务期间持有，Latch仅在操作数据结构时持有
4. Lock需要支持事务回滚，Latch不需要

缓冲池理解
1. 缓冲池是内存中的一块区域，用于存储从磁盘上读取的页
2. 缓冲池会记录每个页元数据，比如是否被修改（脏页）、引用计数（计数大于0的页无法被驱逐）
3. 当缓存池满了，需要通过置换策略来决定哪些页被驱逐出去，比如LRUK算法

缓冲池优化
1. 多缓冲池（每种页类型一个缓冲池，每个数据库一个缓冲池）
2. 预取：根据查询计划提前将页面加载到缓冲池中
3. 扫描共享：多个查询存储数据共用情况时，部分共享页可以不用重复扫描
4. 绕过缓冲池：如顺序扫描需要加载大量页，导致其他常用页被驱逐，则可以绕过缓冲池
